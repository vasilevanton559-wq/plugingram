...
TextWithEntities WithPhoneSpoiler(TextWithEntities phone) {
	if (phone.text.isEmpty()) {
		return phone;
	}
	for (const auto &entity : phone.entities) {
		if (entity.type() == EntityType::Spoiler) {
			return phone;
		}
	}
	return Wrapped(std::move(phone), EntityType::Spoiler);
}

TextWithEntities WithPhoneSpoiler(const QString &phone) {
	return WithPhoneSpoiler(TextWithEntities{ phone });
}
