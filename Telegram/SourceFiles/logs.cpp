...
void start() {
	...
	if (cWorkingDir().isEmpty()) {
		// Plugingram keeps profile data in %AppData%/Plugingram Desktop/
		// (same idea as Telegram Desktop), not next to a random .exe location.
#if defined Q_OS_WIN || defined Q_OS_WINRT || defined OS_WIN_STORE || defined OS_MAC_STORE || !defined _DEBUG
		cForceWorkingDir(psAppDataPath());
#else // debug non-win: allow portable beside exe, fallback to AppData
