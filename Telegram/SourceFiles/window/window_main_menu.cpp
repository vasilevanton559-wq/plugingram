...
, _version(AddVersionLabel(_footer)) {
	...
	_footer->heightValue(
		...
		_channel->moveToLeft(
			...
			_footer->height()
				- st::mainMenuChannelBottom
				- _channel->height());
		const auto telegramTop = _footer->height()
			- st::mainMenuTelegramBottom
			- _telegram->height();
		_telegram->moveToLeft(st::mainMenuFooterLeft, telegramTop);
		_stage->moveToLeft(
			st::mainMenuFooterLeft
				+ _telegram->width()
				+ st::mainMenuStageSkip,
			telegramTop
				+ (_telegram->height() - _stage->height()) / 2);
		_version->moveToLeft(
			st::mainMenuFooterLeft,
			_footer->height()
	...
	_channel->setMarkedText(tr::link(
		u"@plugingram_official"_q,
		u"https://t.me/plugingram_official"_q));
	_channel->setLinksTrusted();
	_telegram->setMarkedText(tr::link(
		u"Plugingram"_q,
		u"https://t.me/plugingram_official"_q));
	_telegram->setLinksTrusted();
	_stage->setText(u"pre-beta"_q);
	_stage->setTextColorOverride(st::windowSubTextFg->c);
	_stage->resizeToNaturalWidth(_stage->textMaxWidth());
