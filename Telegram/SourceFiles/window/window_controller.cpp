...
void Controller::toggleDeveloperOverlay() {
	if (!_developerOverlay) {
		_developerOverlay = base::make_unique_q<DeveloperOverlay>(
			_widget.bodyWidget(),
			this);
	}
	if (_developerOverlay->isShown()) {
		_developerOverlay->hideAnimated();
	} else {
		_developerOverlay->showAnimated();
	}
}
