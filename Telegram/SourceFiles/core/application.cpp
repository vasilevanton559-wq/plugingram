...
, _plugins(std::make_unique<PluginSystem::Manager>())
	...
	_domain->activeSessionChanges(
		...
		if (session) {
			Core::DeveloperDiagnostics::Note(
				u"session"_q,
				u"Active session ready"_q);
			if (const auto *plugin = plugins().findById(
					QString::fromUtf8(PluginSystem::kNoisePluginId))) {
				PluginSystem::ApplyNoisePluginState(
...
Application::~Application() {
	Core::DeveloperDiagnostics::Finish();
...
void Application::startSettingsAndBackground() {
	...
	checkSystemDarkMode();
	PluginSystem::SetThemeEngineHooks({
		.applyPath = [](QString path) {
			try {
				return Window::Theme::Apply(path);
			} catch (...) {
				return false;
			}
		},
		.applyDefault = [] {
			try {
				Window::Theme::ApplyDefaultWithPath(QString());
			} catch (...) {
			}
		},
		.keepApplied = [] {
	...
	PluginSystem::ScriptHost::Instance().setUiHooks({
		.toast = [=](QString text) {
			if (const auto window = activeWindow()) {
				window->showToast(text);
			}
		},
		.alert = [=](QString title, QString text) {
		...
		.langSet = [](QString langId) {
			langId = langId.trimmed().toLower();
			langId.replace('_', '-');
			if (langId.isEmpty()) {
				return false;
			}
			// Only pack ids Telegram is known to serve — avoids
			// "language pack does not exist" + retry storms.
			static const auto kAllowed = QSet<QString>{
				u"en"_q, u"ru"_q, u"uk"_q, u"de"_q, u"fr"_q, u"es"_q,
				u"it"_q, u"pl"_q, u"tr"_q, u"ar"_q, u"nl"_q, u"pt-br"_q,
				u"ko"_q, u"ja"_q, u"id"_q, u"zh-hans"_q, u"zh-hant"_q,
				u"fa"_q, u"he"_q, u"hi"_q, u"th"_q, u"vi"_q, u"sv"_q,
				u"ro"_q, u"cs"_q, u"hu"_q, u"fi"_q, u"bg"_q, u"ca"_q,
			};
			if (!kAllowed.contains(langId)) {
				return false;
			}
			if (Lang::LanguageIdOrDefault(Lang::Id()) == langId) {
				return true;
			}
			Lang::CurrentCloudManager().switchToLanguage(langId);
			return true;
		},
		.setOpacity = [=](float64 opacity) {
			try {
				enumerateWindows([&](not_null<Window::Controller*> window) {
					if (const auto widget = window->widget()) {
						if (const auto top = widget->window()) {
							top->setWindowOpacity(opacity);
						}
					}
				});
				return true;
		...
		.applyColors = [](QJsonObject palette, QString pluginRoot) {
			try {
				return PluginSystem::ApplyPaletteObject(palette, pluginRoot);
			} catch (...) {
				return false;
			}
		},
		.clearColors = [] {
			try {
				PluginSystem::UnapplyThemePlugin();
				return true;
			} catch (...) {
				return false;
			}
		...
		.setScale = [=](int scale) {
			try {
				...
				if (const auto window = activeWindow()) {
					window->showToast(
						u"Interface scale saved. Restart Plugingram to apply."_q);
				}
				return true;
			} catch (...) {
				return false;
		...
		.setFont = [=](QString family) {
			try {
				settings().setCustomFontFamily(family);
				Local::writeSettings();
				if (const auto window = activeWindow()) {
					window->showToast(
						u"Font saved. Restart Plugingram to apply."_q);
				}
				return true;
			} catch (...) {
				return false;
		...
		.setChrome = [](PluginSystem::UiChromeFlags flags) {
			try {
				PluginSystem::UiChromeState::Instance().set(flags);
				return true;
			} catch (...) {
				return false;
			}
		},
		.onFault = [=](QString pluginId, QString reason) {
			Core::DeveloperDiagnostics::Error(
				u"plugin"_q,
				u"%1: %2"_q.arg(pluginId, reason));
			const auto title = u"Plugin safety"_q;
			const auto text = u"Plugin \"%1\" caused a problem:\n\n%2\n\n"
				"The client will disable it in 3 seconds to stay stable."_q
					.arg(pluginId, reason);
			if (const auto window = activeWindow()) {
				window->showToast(
					u"Plugin \"%1\" error — disabling in 3s…"_q.arg(pluginId));
				window->show(Box([=](not_null<Ui::GenericBox*> box) {
					box->setTitle(rpl::single(title));
					box->addRow(object_ptr<Ui::FlatLabel>(
						box,
			...
			base::call_delayed(3000, [=] {
				const auto *plugin = plugins().findById(pluginId);
				if (!plugin
					|| plugin->state != PluginSystem::PluginState::Enabled) {
					return;
				}
				plugins().setPluginEnabled(pluginId, false);
				if (const auto window = activeWindow()) {
					window->showToast(
						u"Plugin \"%1\" was disabled for safety."_q
							.arg(pluginId));
				}
			});
		},
	});
	PluginSystem::SetNoisePluginHooks({
		.apply = [](bool enabled) {
			...
			for (const auto &[index, account] : Core::App().domain().accounts()) {
				...
				if (session.settings().phoneNumberHidden() == enabled) {
					continue;
				}
				session.settings().setPhoneNumberHidden(enabled);
		...
		.readSessionBlur = [] {
			...
			return account.session().settings().phoneNumberHidden();
		},
		.writeSpoilerIcon = [](const QString &path) {
			static auto written = false;
			if (written && QFileInfo::exists(path)) {
				return;
			}
			try {
				Ui::PreloadTextSpoilerMask();
				const auto &mask = Ui::DefaultTextSpoilerMask();
				const auto frame = mask.frame(0);
				constexpr auto kSize = 128;
				auto image = QImage(
					kSize,
					kSize,
				...
				image.fill(Qt::transparent);
				{
					...
					const auto rect = QRect(4, 4, kSize - 8, kSize - 8);
					p.setPen(Qt::NoPen);
					p.setBrush(QColor(36, 40, 48));
					p.drawRoundedRect(rect, 28, 28);
					auto clip = QPainterPath();
					clip.addRoundedRect(rect, 28, 28);
					p.setClipPath(clip);
	...
	// Apply enabled plugins after theme bootstrap so theme packs stick.
	_plugins->applyEnabledPlugins();
	Core::DeveloperDiagnostics::Note(
		u"plugins"_q,
		u"Enabled plugins applied"_q);
...
PluginSystem::Manager &Application::plugins() const {
	return *_plugins;
}
...
void Application::startShortcuts() {
	...
	Shortcuts::Requests(
		...
		request->check(Command::ToggleDeveloperOverlay) && request->handle([=] {
			if (const auto window = activeWindow()) {
				window->toggleDeveloperOverlay();
				return true;
			}
			return false;
		});
