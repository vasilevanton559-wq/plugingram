...
PreLaunchWindow::PreLaunchWindow(QString title) {
	...
	setWindowTitle(title.isEmpty() ? u"Plugingram"_q : title);
...
, _log(this)
, _close(this) {
	_label.setText(u"Could not start Plugingram!\nYou can see complete log below:"_q);
...
, _launch(std::move(launch)) {
	...
	if (_sendingState == SendingNoReport) {
		_label.setText(u"Last time Plugingram was not closed properly."_q);
	} else {
		_label.setText(u"Last time Plugingram crashed :("_q);
	}
...
void LastCrashedWindow::checkingFinished() {
	...
	if (result == "Old") {
		_pleaseSendReport.setText(u"This report is about some old version of Plugingram."_q);
		_sendingState = SendingTooOld;
		updateControls();
		return;
	} else if (result == "Unofficial") {
		_pleaseSendReport.setText(u"You use some custom version of Plugingram."_q);
...
void LastCrashedWindow::updateControls() {
	...
	QSize s(2 * padding + QFontMetrics(_label.font()).horizontalAdvance(u"Last time Plugingram was not closed properly."_q) + padding + _networkSettings.width(), h);
	if (s == size()) {
		resizeEvent(0);
	} else {
		resize(s);
