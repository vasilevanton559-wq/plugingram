...
, _checkRequestTimer([=] { checkRequest(); }) {
	...
	updateDescText();
	setTitleMarkedText(_isFragment.value(
	) | rpl::map([=](bool isFragment) -> rpl::producer<TextWithEntities> {
		return !isFragment
			? rpl::single(Ui::Text::WithPhoneSpoiler(
				Ui::FormatPhone(getData()->phone)))
			: (tr::lng_intro_fragment_title() | rpl::map(tr::marked));
	}) | rpl::flatten_latest());
