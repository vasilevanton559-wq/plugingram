...
, _description(
	...
	_descriptionText.value(
		...
		// Phone spoilers stay clickable; other spoiler-only hints (e.g. email
		// patterns) keep mouse events disabled so they can't be revealed.
		const auto phoneLike = text.text.contains('+')
			|| text.text.contains(QRegularExpression(u"\\d{5,}"_q));
		label->setAttribute(
			Qt::WA_TransparentForMouseEvents,
			hasSpoiler && !phoneLike);
...
void Step::setTitleMarkedText(rpl::producer<TextWithEntities> titleText) {
	std::move(
		titleText
	) | rpl::on_next([=](const TextWithEntities &text) {
		_title->setMarkedText(text);
