...
void ComposeControls::syncPluginComposerButtons() {
	if (!_wrap || _wrap->width() <= 0) {
		return;
	}
	auto key = QString();
	struct Item {
	...
	for (const auto &panel
		...
		if (panel.placement != u"chat.composer"_q) {
			continue;
		}
		if (panel.actions.empty()) {
			...
			for (const auto &action : panel.actions) {
				items.push_back({
					.pluginId = panel.pluginId,
					.actionId = action.id,
					.label = action.title.isEmpty()
						? action.id
						: action.title,
				});
				key += panel.pluginId + '/' + action.id + ';';
			}
		}
		if (int(items.size()) >= 3) {
			break;
	...
	if (key == _pluginButtonsKey) {
		return;
	}
	_pluginButtonsKey = key;
	_pluginButtons.clear();
	_pluginButtonsWidth = 0;
	for (const auto &item : items) {
		...
		const auto pluginId = item.pluginId;
		const auto actionId = item.actionId;
		button->setClickedCallback([=] {
			try {
				Core::App().plugins().runUtilityCommand(pluginId, actionId);
			} catch (...) {
			}
		});
		button->show();
		_pluginButtonsWidth += button->width();
		_pluginButtons.push_back(std::move(button));
	}
}
...
void ComposeControls::updateControlsGeometry(QSize size) {
	...
	// Rebuild plugin buttons outside the geometry stack to avoid re-entrancy.
	{
		auto key = QString();
		for (const auto &panel
				: PluginSystem::UiExtensionRegistry::Instance().panels()) {
		...
		if (key != _pluginButtonsKey && _wrap) {
			InvokeQueued(_wrap.get(), [=] {
				syncPluginComposerButtons();
				if (_wrap) {
					updateControlsGeometry(_wrap->size());
				}
			});
	...
	// Drop dangling QPointer entries after parent rebuilds.
	{
		auto compact = std::vector<base::unique_qptr<Ui::IconButton>>();
		compact.reserve(_pluginButtons.size());
		_pluginButtonsWidth = 0;
		for (auto &button : _pluginButtons) {
			if (!button) {
				continue;
			}
			_pluginButtonsWidth += button->width();
			compact.push_back(std::move(button));
		}
		_pluginButtons = std::move(compact);
	}
	...
	const auto fieldWidth = std::max(
		...
		- (commentsShown
			...
			: 0)
		- ((_attachToggle || _sendAs || _pluginButtonsWidth > 0)
			? _st.padding.left()
			: _st.fieldLeft)
	...
	left += (_attachToggle || _sendAs || _pluginButtonsWidth > 0)
		? _st.padding.left()
		: _st.fieldLeft;
	if (_replaceMedia) {
	...
	for (const auto &button : _pluginButtons) {
		if (!button) {
			continue;
		}
		button->moveToLeft(left, buttonsTop);
		left += button->width();
	}
	if (_sendAs) {
