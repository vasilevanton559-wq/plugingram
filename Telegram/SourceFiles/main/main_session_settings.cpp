...
QByteArray SessionSettings::serialize() const {
	...
	result.reserve(size);
	{
		...
		// 1 = blur on, 2 = explicitly off (0 is legacy "never set").
		stream << qint32(_phoneNumberHidden.current() ? 1 : 2);
...
void SessionSettings::addFromSerialized(const QByteArray &serialized) {
	...
	// Plugingram: legacy installs stored 0 (off). Enable blur by default;
	// user can still disable via phone context menu (persists as 0 after).
	// Use 2 as "explicitly disabled" so first upgrade turns blur on.
	if (phoneNumberHidden == 2) {
		_phoneNumberHidden = false;
	} else {
		_phoneNumberHidden = true;
	}
