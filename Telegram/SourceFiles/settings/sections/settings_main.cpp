...
void Cover::updatePhoneText() {
	...
	} else if (_user->session().settings().phoneNumberHidden()) {
		// Native spoiler noise; click reveals like message spoilers.
		_phone->setMarkedText(
			Ui::Text::WithPhoneSpoiler(_phoneText));
	} else {
		_phone->setText(_phoneText);
	}
...
void BuildSectionButtons(SectionBuilder &builder) {
	...
	if (const auto pluginsButton = builder.addSectionButton({
		.title = tr::lng_settings_plugins(),
		.targetSection = PluginsId(),
		.icon = { &st::menuIconShop },
		.keywords = { u"plugins"_q, u"modules"_q, u"store"_q, u"extensions"_q },
	})) {
		ApplyPluginsRainbowToButton(pluginsButton);
	}
	...
	for (const auto &panel
		...
		builder.addButton({
			.id = u"main/plugin-panel/"_q + panel.pluginId + u'/' + panel.id,
			.title = rpl::single(panel.title),
			.icon = { &st::menuIconCustomize },
			.label = rpl::single(panel.pluginName),
			.onClick = [controller, panel] {
				controller->show(Box([=](not_null<Ui::GenericBox*> box) {
					box->setTitle(rpl::single(panel.title));
					box->addRow(object_ptr<Ui::FlatLabel>(
						box,
						panel.description.isEmpty()
							? QStringLiteral("Panel from %1").arg(
								panel.pluginName)
							: panel.description,
						st::boxLabel));
					for (const auto &action : panel.actions) {
						const auto pluginId = panel.pluginId;
						const auto commandId = action.id;
						const auto title = action.title;
						box->addRow(object_ptr<Ui::SettingsButton>(
							box,
							rpl::single(title),
...
void BuildHelpSection(SectionBuilder &builder) {
	...
	if (const auto featuresButton = builder.addButton({
		.id = u"main/plugingram-features"_q,
		.title = rpl::single(u"Возможности Plugingram"_q),
		.onClick = [=] {
			controller->show(Box(PlugingramFeaturesBox));
		},
		.keywords = {
			u"plugingram"_q,
			u"features"_q,
		...
		ApplyPlugingramFeaturesRainbowToButton(featuresButton);
	}

	builder.addButton({
		.id = u"main/support-creator"_q,
		.title = rpl::single(u"Поддержать создателя"_q),
		.icon = { &st::menuIconYooMoney, IconType::Simple },
		.onClick = [=] {
			controller->show(Box(SupportCreatorBox));
		},
		.keywords = {
			u"donate"_q,
			u"support"_q,
