...
rpl::producer<TextWithEntities> PhoneWithSpoilerValue(
	...
	// Built-in privacy: blur every visible phone with Telegram's native
	// spoiler noise (same particles as spoiler text/media). Click reveals.
	// Uncheck "Spoiler effect" in the phone context menu to show numbers.
	return rpl::combine(
		std::move(phone),
		user->session().settings().phoneNumberHiddenValue()
	) | rpl::map([](TextWithEntities phone, bool blur) {
		if (!blur
			|| phone.text.isEmpty()
			|| phone.text == tr::lng_info_mobile_hidden(tr::now)) {
			return phone;
		}
		return Ui::Text::WithPhoneSpoiler(std::move(phone));
	});
}
