...
void AddPhoneSpoilerMenu(
	...
	const auto toggle = [=] {
		const auto next = !session->settings().phoneNumberHidden();
		// Keep Plugins toggle and session setting in sync.
		if (!Core::App().plugins().setPluginEnabled(
				QString::fromUtf8(PluginSystem::kNoisePluginId),
				next)) {
			auto &settings = session->settings();
			settings.setPhoneNumberHidden(next);
			session->saveSettingsDelayed();
		}
	};
	// Global client preference: blur own + others' phones in profiles.
