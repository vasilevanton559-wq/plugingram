...
void LayerWidget::parentResized() {
	...
	//	takeToThirdSection();
		...
		// Peer Profile Info is not Settings — never call settingsType() there
		// (Expects Type::Settings and would abort the client).
		const auto &section = _contentWrap->controller()->section();
		const auto pluginsWide = (section.type() == Section::Type::Settings)
			&& (section.settingsType() == ::Settings::PluginsId());
		const auto desiredWidth = pluginsWide
			? std::max(st::infoDesiredWidth, style::ConvertScale(780))
			: st::infoDesiredWidth;
		auto newWidth = qMin(
...
bool LayerWidget::showSectionInternal(
	...
	if (_contentWrap && _contentWrap->showInternal(memento, params)) {
		...
		// Recompute layer width (Plugins uses a wider landscape panel).
		parentResized();
		return true;
	}
	return false;
