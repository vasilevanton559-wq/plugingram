...
void PeerShortInfoBox::prepareRows() {
	...
	addInfoOneLine(
		...
		linkValue(),
		tr::lng_context_copy_link(tr::now));
	auto phoneBlurValue = [&]() -> rpl::producer<bool> {
		if (Core::App().domain().started()
			&& Core::App().domain().active().sessionExists()) {
			return Core::App().domain().active().session().settings(
	...
	addInfoOneLine(
		...
		rpl::combine(
			phoneValue(),
			std::move(phoneBlurValue)
		) | rpl::map([](QString &&phone, bool blur) {
			auto marked = TextWithEntities{ std::move(phone) };
			if (!blur || marked.text.isEmpty()) {
				return marked;
			}
			return Ui::Text::WithPhoneSpoiler(std::move(marked));
		}),
