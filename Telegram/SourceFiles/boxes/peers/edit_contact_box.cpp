...
, _userpic(
	...
	// Keep mouse events so native phone spoiler can be revealed on click.
...
void Cover::initViewers(rpl::producer<QString> status) {
	...
	) | rpl::on_next([=](const QString &status, bool blur) {
		if (status.isEmpty()
			|| status == tr::lng_contact_mobile_hidden(tr::now)) {
			_status->setText(status);
		} else if (blur) {
			_status->setMarkedText(Ui::Text::WithPhoneSpoiler(status));
		} else {
			_status->setText(status);
		}
