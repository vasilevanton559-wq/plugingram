...
, _singleMessageSearch(&controller->session()) {
	...
	// Child dialogs lists (forum/channel topic lists) must not run chrome
	// hooks — they crash when search/mainMenu/filters are touched.
	if (_layout != Layout::Child) {
		rpl::single(rpl::empty) | rpl::then(
			PluginSystem::UiChromeState::Instance().changes()
		) | rpl::on_next([=] {
			InvokeQueued(this, [=] {
				applyPluginChromeVisibility();
			});
		}, lifetime());
	}
}
...
void Widget::applyPluginChromeVisibility() {
	if (_layout == Layout::Child) {
		return;
	}
	const auto flags = PluginSystem::UiChromeState::Instance().flags();
	const auto wantFilters = flags.filters
		&& session().settings().dialogsFiltersEnabled()
		&& (Core::App().settings().chatFiltersHorizontal()
			|| !controller()->enoughSpaceForFilters());
	toggleFiltersMenu(wantFilters);
	const auto filtersHidden = !controller()->filtersWidth();
	const auto inSubsection = (_openedFolder != nullptr)
		|| (_openedForum != nullptr)
		|| (_openedCommunity != nullptr);
	_mainMenu.toggle->setVisible(
		flags.mainMenu && filtersHidden && !inSubsection);
	_mainMenu.under->setVisible(
		flags.mainMenu && filtersHidden && !inSubsection);
	// Never force-show search while a folder/forum/community header is open.
	_search->setVisible(flags.search && !inSubsection);
	if (!flags.search || inSubsection) {
		_cancelSearch->toggle(false, anim::type::instant);
		_jumpToDate->toggle(false, anim::type::instant);
		_chooseFromUser->toggle(false, anim::type::instant);
	}
