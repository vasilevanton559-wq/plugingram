...
void EnsureInstalledInAppData() {
	const auto dataDir = psAppDataPath();
	if (dataDir.isEmpty() || cExeName().isEmpty()) {
		return;
	}
	QDir().mkpath(dataDir);
	QDir().mkpath(dataDir + u"plugins"_q);
	QDir().mkpath(dataDir + u"tdata"_q);

	const auto currentExe = QFileInfo(
		cExeDir() + cExeName()).absoluteFilePath();
	const auto targetExe = QFileInfo(
		dataDir + cExeName()).absoluteFilePath();
	if (QDir::toNativeSeparators(currentExe).compare(
			QDir::toNativeSeparators(targetExe),
			Qt::CaseInsensitive) != 0) {
		const auto currentInfo = QFileInfo(currentExe);
		const auto targetInfo = QFileInfo(targetExe);
		const auto needCopy = !targetInfo.exists()
			|| targetInfo.size() != currentInfo.size()
			|| targetInfo.lastModified() < currentInfo.lastModified();
		if (needCopy) {
			QFile::remove(targetExe);
			if (!QFile::copy(currentExe, targetExe)) {
				LOG(("App Error: could not install exe to %1").arg(targetExe));
			} else {
				LOG(("Installed exe to %1").arg(targetExe));
			}
		}
	}

	// Desktop shortcut -> AppData install, so the Desktop stays clean.
	...
	const auto hr = SHGetKnownFolderPath(
		...
		nullptr,
		&desktopFolder);
	if (!SUCCEEDED(hr) || !desktopFolder) {
		return;
	}
	const auto guard = gsl::finally([&] {
		CoTaskMemFree(desktopFolder);
	});
	const auto lnk = QString::fromWCharArray(desktopFolder)
		+ u"\\"_q
		+ AppName.utf16()
		+ u".lnk"_q;
	const auto shellLink = base::WinRT::TryCreateInstance<IShellLink>(
	...
	const auto exeNative = QDir::toNativeSeparators(targetExe);
	...
	shellLink->SetDescription(AppName.utf16().toStdWString().c_str());
	if (const auto propertyStore = shellLink.try_as<IPropertyStore>()) {
		PROPVARIANT appIdPropVar;
		if (SUCCEEDED(InitPropVariantFromString(
				AppUserModelId::Id().c_str(),
				&appIdPropVar))) {
			propertyStore->SetValue(AppUserModelId::Key(), appIdPropVar);
			PropVariantClear(&appIdPropVar);
			propertyStore->Commit();
		}
	}
	if (const auto persistFile = shellLink.try_as<IPersistFile>()) {
		persistFile->Save(lnk.toStdWString().c_str(), TRUE);
	}
}
