		...
		do {
			...
			if (fname.substr(0, tdataDir.size()) == tdataDir && (fname.size() <= tdataDir.size() || fname.at(tdataDir.size()) == '/')) {
				...
				if (equal(tofname, updaterName)) { // bad update - has Updater.exe - delete all dir
					...
					delFolder();
					return false;
				} else if (equal(tofname, updateTo + L"Plugingram.exe") && exeName != L"Plugingram.exe") {
...
static const WCHAR *_programName = L"Plugingram"; // folder in APPDATA, if current path is unavailable for writing
